...
% numeric array.
[data,~]=text2bin('Hello World!');      %Tx of Content
numTrBlks = 50;              % Number of simulated transport blocks
...
Power   = [-10:2:-2 -1.9:0.1:3.9 4:2:30];
itr=100;

for cnt = 1:itr
    clearvars DeltaX DeltaY DeltaZ Rk k axang Local_Sat el_iot az_iot x_iot y_iot z_iot sumIQ outputIQ ChosenObserver Serveidx
    ...
    while numel(Servingcell) == 0;
        ...
        ObserverTimeline = OnDuty(:,ChosenObserver,:);         % the time period when the observer being serve by any satellites
    ...
    chsInit.NRep = simReps; % Number of repetitions of the NPUSCH
    ...
    for PowerIdx = 1:numel(Power)
        ...
        for slotIdx = 0+(0:TotNSlots-1)
            ...
            outputIQ = [zeros(1,5000) outputIQ'] ;
            ...
            RxIQ =  sumIQ(:,5000:5000+ObserverPacketLength);
            % Create additive white Gaussian noise
            
            noise = wgn(2,length(RxIQ),N0/2,'linear')+1i * wgn(2,length(RxIQ),N0/2,'linear');
...
save('PERvsTxpowerCase3_NBIoT_Rep1');
